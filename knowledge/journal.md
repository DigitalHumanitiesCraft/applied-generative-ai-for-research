...
## 2026-08-22 — The introduction and parts I to III written from the teaching sources

The two Full Lecture Notes and the slide-text export were distilled, and after the operator cleared it for publication the vault document "Vault als materialisiertes Wissensmodell" followed. The German notes carry the chapter numbering and were distilled in full, including their Promptotyping chapter, so that the other manuscript lane can ground in them. The English notes and the deck were distilled only where they add wording or material the German version does not carry, and each of the three distillates says so in its lead and links the parallel one, which keeps the same statement from entering the chain twice under two identifiers. What the English version adds is the account of model internals from tokenisation to interpretability, the model and harness landscape, the history of the agent concept and the four-part assessment vocabulary. What the deck adds sits in its speaker notes, the two sources of Knowledge Engineering, the shifted formalisation target and the reading of the knowledge system as a production system for target artefacts.

Assertions here overlap in their grounding without nesting, which differs from the strict partition the Promptotyping lane adopted. Two assertions may rest on a shared statement as long as each set holds an element the other lacks, so `W-DUPLICATE-GROUNDING` stays silent while a statement that genuinely supports two different claims can be cited by both. The partition rule would have forced a duplicate statement in the distillate for every such case, which would have put the same source passage under two identifiers and made the fidelity check ambiguous. The looser rule was checked by the validator over the whole vault rather than trusted.

The seven conditions of the Grounded Vault were grounded one at a time in chapter 7 rather than asserted as a set, and the result is uneven on purpose. Inspectable, revisable, agent-operable, layered and versioned follow from the sources. Governance appears in them as a demand without an enforcement mechanism, so the chapter reads the condition as satisfied only where behavioural guidance and technical permission are separated, and marks that reading as this book's own criterion. Source-boundedness is the condition the sources reach for and do not supply, because none of them describes a chain that binds an individual statement to its supporting passage; the chapter names that chain as the extension this manuscript adds to the practice it describes, which is also the mechanism the manuscript itself is written with.

Two terms in these chapters are coinages and are marked as such at the point of use. Verification debt in chapter 10 has no anchor in any source of this lane and is assembled from the assertions on purpose-bound acceptance and the four assessment levels. The layered diagnosis of a defect is stated there as a question rather than a method, because the sources establish that a defect can originate outside the implementation without giving a procedure for locating it, and the diagnostic grid of the document typology in Part IV is where the two meet.

Every chapter ends with a gap list whose lead sentence carries a posit footnote and whose bullets stand in the same block, so the list satisfies the anchor contract without pretending to be grounded. The recurring gaps are the same across the eleven chapters. Quantitative claims about model behaviour rest on the secondary report of the lecture notes and need the underlying publications as records of their own, four failure modes the outline names have no anchor at all, and the distinction between curated and deterministically generated knowledge belongs to the document typology of the other lane. The register in [[knowledge/state]] carries the writing status of the eleven chapters, and the gap lists carry what the next pass has to close.
